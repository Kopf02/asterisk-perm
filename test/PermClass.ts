...
describe('PermClass integration test', () => {
  ...
  it('set test.test2 to true', () => {
    ...
    assert.deepStrictEqual(
      permClass.obj,
      { test: { test2: true, '*': true, _: false } },
      'Permission Object not updated' + ' correctly'
    );
  });
  ...
  it('unset test.*', () => {
    ...
    assert.strictEqual(res, true, 'Permission key does not exist');
    assert.deepStrictEqual(permClass.obj, { test: { _: true } }, 'Permission Object not updated' + ' correctly');
  });
  it('unset test', () => {
    const res = permClass.unset('test', true);
    assert.strictEqual(res, true, 'Permission key does not exist');
    assert.deepStrictEqual(permClass.obj, {}, 'Permission Object not updated' + ' correctly');
  });
  //SET AGAIN
  it('set test.test2', () => {
    const res = permClass.set('test.test2', true, true);
    assert.strictEqual(res, true, 'Permission was updated, but should be newly created');
    assert.deepStrictEqual(permClass.obj, { test: { test2: true } }, 'Permission Object not updated' + ' correctly');
  });
  ...
  describe('fromArray', () => {
    ...
    it('Check if object gets converted to json correctly', () => {
      ...
      assert.deepStrictEqual(
        res.obj,
        { test: { test: { '*': true }, test2: false } },
        'Array should match json object'
      );
    });
