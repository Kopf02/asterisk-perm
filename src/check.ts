...
function checkRec(permission: string[], perms: PermObject | boolean, wildcard = false): PermAction | null {
  if (typeof perms === 'boolean') {
    return { action: perms, depth: permission.length };
  }

  if (permission[0] === '$') {
    const results: PermAction[] = [];
    for (const key in perms) {
      if (Object.getOwnPropertyDescriptor(perms, key) && key !== '_') {
        const res = checkRec(permission.slice(1), perms[key], key === '*');
        if (res) results.push(res);
      }
    }
    return results.reduce((prev, curr) => (curr.depth < prev.depth || prev.depth === -1 ? curr : prev), {
      depth: -1,
      action: false,
    });
  }
  ...
  if (permission.length > 0 && perms[permission[0]] !== undefined) {
    const res = checkRec(permission.slice(1), perms[permission[0]]);
    if (res) return res;
  }
...
export default function (permission: string, perms: PermObject): boolean {
  const res = checkRec(permission.split('.'), perms);
