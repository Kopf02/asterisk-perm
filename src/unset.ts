 ...
 * @return boolean True if Removed, False if permission didn't exist
 */
export default function (permission: string, perms: PermObject): boolean {
  const permArray = permission.split('.');
  const handled: { permission: string; perms: PermObject }[] = [];
  ...
  while (handled.length > 0) {
    const clean = handled.pop();
    const length = Object.keys(clean.perms[clean.permission]).length;
